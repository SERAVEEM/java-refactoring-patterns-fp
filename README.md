...
**Perancang Arsitektur:** Senior Software Developer / Architect  
...
## Daftar Isi
- [Ringkasan Eksekutif Arsitektur](#ringkasan-eksekutif-arsitektur)
- [Ilustrasi Aliran Data (Data Flow)](#ilustrasi-aliran-data-data-flow)
- [Perbandingan Struktur Sistem (Before vs After)](#perbandingan-struktur-sistem-before-vs-after)
- [Identifikasi Code Smells dan Langkah Refactoring](#identifikasi-code-smells-dan-langkah-refactoring)
  - [Modul: Customer.java](#modul-customerjava)
  - [Modul: Rental.java](#modul-rentaljava)
  - [Modul: Movie.java & Price Strategy](#modul-moviejava--price-strategy)
- [Verifikasi Pengujian Otomatis (Black-Box Testing)](#verifikasi-pengujian-otomatis-black-box-testing)
...
Melalui rangkaian proses rekayasa ulang ini, aliran data dibalik dari pendekatan tradisional *"Ask, Then Do"* menjadi *"Tell, Don't Ask"*. Selain itu, struktur `switch‑case` digantikan secara menyeluruh menggunakan konsep **Polimorfisme** berbasis **State/Strategy Pattern**.
...
```
*Weakness: Kelas `Customer` harus mengetahui detail rumus perhitungan harga semua jenis film.*
    ...
    +---→ Iterasi Daftar Sewa ---→ [Rental.getCharge()]
                               ...
                               +---→ Delegasi ke Objek Film ---→ [Movie.getCharge(daysRented)]
                                                           ...
                                                           +---→ Polimorfisme Strategi Harga ---→ [Price.getCharge(daysRented)]
                                                               ...
                                                               |--- [NewReleasePrice]
```
*Benefit: Setiap kelas bertanggung jawab penuh atas validitas datanya sendiri. Kelas `Customer` hanya menerima hasil akhir berupa nilai numerik untuk dicetak.*
...
- **Switch Statements (Gejala Klinis Terselubung)**: Meskipun logika `switch` berada di kelas lain, keberadaan metode `getPriceCode()` menjadi akar munculnya struktur `switch‑case` berulang di seluruh sistem, melanggar **Open/Closed Principle**.

#### Strategi Refactoring yang Dipilih
Mengganti nilai kode numerik konvensional dengan objek kelas konkrit melalui implementasi **State/Strategy Pattern**. Pola ini mengisolasi variasi kalkulasi harga ke dalam sub‑kelas yang independen sehingga penambahan tipe film baru tidak memerlukan perubahan kode yang sudah mapan.

#### Langkah‑Langkah Mekanis
1. Menginisialisasi kelas abstrak induk baru bernama `Price.java`.
2. Membuat sub‑kelas turunan yang spesifik mengimplementasikan rumus matematika masing‑masing tipe film: `RegularPrice.java`, `ChildrenPrice.java`, dan `NewReleasePrice.java`.
3. Mengubah atribut `int priceCode` pada kelas `Movie.java` menjadi referensi objek `Price price`.
4. Mengkonfigurasi metode `setPriceCode(int code)` sebagai pusat alokasi pembentukan objek strategi harga.
...
Untuk menjamin tingkat keandalan sistem pasca‑refactoring, metode pengujian **Black‑Box Testing** diaplikasikan dengan teknik **Equivalence Partitioning**. Seluruh skenario pengujian diotomatisasi secara ketket melalui framework **JUnit** dalam unit tes `VideoStoreTest.java`.
